...
let
  cfg = config.nixModules.firefox;
in
{
  options.nixModules.firefox = {
    containers = lib.mkOption {
      type = lib.types.attrsOf lib.types.attrs;
      default = { };
      description = ''
        Containers for the main profile, in the shape of
        programs.firefox.profiles.<name>.containers. Declared as an option so
        personal container sets stay out of this public module. Give each
        container a stable id: Multi-Account Containers site assignments
  ...
  config = {
    ...
    programs.firefox = {
      ...
      profiles.${config.home.username} = {
        ...
        containersForce = cfg.containers != { };
        ...
        extensions.packages =
          with nixModulesInputs.firefox-addons.packages.${pkgs.stdenv.hostPlatform.system}; [
            canvasblocker
            decentraleyes
            floccus
